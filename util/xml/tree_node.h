...
class TreeNode {
 public:
  TreeNode(const std::string& name, T* parent)
      : name_(name)
      , parent_(parent)
      , first_child_(NULL)
      , last_child_(NULL)
  ...
  T* pointer() { return static_cast<T*>(this);}
...
template<class T>
T* TreeNode<T>::find(const std::string& name) {
  if (name_ == name) return static_cast<T*>(this);
  T* node = first_child();
...
void TreeNode<T>::InsertBefore(T *before) {
  ...
  if (before->prev_sibling()) {
    before->prev_sibling_->next_sibling_ = static_cast<T*>(this);
    this->prev_sibling_ = before->prev_sibling();
    before->prev_sibling_ = static_cast<T*>(this);
  } else {
    this->prev_sibling_ = NULL;
    before->prev_sibling_ = static_cast<T*>(this);
    before->parent_->first_child_ = static_cast<T*>(this);
  }
}
...
void TreeNode<T>::InsertAfter(T *after) {
  ...
  if (after->next_sibling()) {
    ...
    this->next_sibling_->prev_sibling_ = static_cast<T*>(this);
    after->next_sibling_ = static_cast<T*>(this);
  } else {
    after->next_sibling_ = static_cast<T*>(this);
    this->next_sibling_ = NULL;
    this->parent_->last_child_ = static_cast<T*>(this);
  }
}

template<class T>
void TreeNode<T>::traverse(Traverser* traverse) {
  if (!traverse->OnVisitBegin(static_cast<T*>(this))) return;
  ...
  traverse->OnVisitEnd(static_cast<T*>(this));
}
