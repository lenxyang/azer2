...
#include "azer/render/skia/util.h"
#include "SkMatrix.h"
#include <stdio.h>
...
void GrGLCheckErr(const GrGLInterface* gl,
    ...
    if (GR_GL_NO_ERROR != err) {
        std::stringstream ss;
        ss << StringPrintf("---- glGetError 0x%x(%s)", err, get_error_string(err));
        if (location) {
            ss << (StringPrintf(" at\n\t%s", location));
        }
        if (call) {
...
GrGLStandard GrGLGetStandardInUseFromString(const char* versionString) {
    if (NULL == versionString) {
        DLOG(ERROR) << ("NULL GL version string.");
...
GrGLSLVersion GrGLGetGLSLVersionFromString(const char* versionString) {
    if (NULL == versionString) {
        DLOG(ERROR) << ("NULL GLSL version string.");
