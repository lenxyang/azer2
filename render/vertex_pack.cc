...
VertexPack::VertexPack(SlotVertexData* data)
  ...
  vertex_data_->set_slot_vertex_data(data, 0);
}
...
void VertexPack::WriteIntVec4(const IntVec4& v, const VertexPos& pos) {
  ...
  if (get_data_type(pos) == kIntVec4) {
    ...
    *vec = v;
    ...
    uint32_t* ptr = (uint32_t*)get_data_ptr(pos);
...
void VertexPack::WriteVector4(const Vector4& v, const VertexPos& pos) {
  ...
  if (get_data_type(pos) == kVec4) {
    ...
    *vec = v;
    ...
    uint32_t* ptr = (uint32_t*)get_data_ptr(pos);
    uint32_t x = ((uint32_t)(v.x * 255));
    uint32_t y = ((uint32_t)(v.y * 255)) >> 8;
    uint32_t z = ((uint32_t)(v.z * 255)) >> 16;
    uint32_t w = ((uint32_t)(v.w * 255)) >> 24;
    *ptr = x | y | z | w;
  } else {
    CHECK(false);
  }
...
void VertexPack::ReadInt(int32_t* v, const VertexPos& pos) const {
  CHECK(pos.slot != -1 && pos.index != -1);
  DCHECK(get_data_type(pos) == kInt);
  int32_t* ptr = (int32_t*)(get_data_ptr(pos));
  *v = *ptr;
}
...
void VertexPack::ReadIntVec4(IntVec4* v, const VertexPos& pos) const {
  ...
  if (get_data_type(pos) == kIntVec4) {
    ...
    *v = *ptr;
    ...
    int32_t i32 = *(int32_t*)(get_data_ptr(pos));
    v->x = (0x000000ff & i32);
...
uint8_t* VertexPack::get_data_ptr(const VertexPos& pos) const {
  SlotVertexData* slot = vertex_data_->vertex_data_at(pos.slot);
  uint8_t* ptr = slot->vertex_data_at(index_);
...
bool GetSemanticIndex(const std::string& name, int32_t semantic_index, 
  ...
  for (int32_t i = 0; i < d->element_count(); ++i) {
    if (base::CompareCaseInsensitiveASCII(name, (desc + i)->name) == 0
