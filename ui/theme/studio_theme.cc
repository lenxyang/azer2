...
#include "azer/ui/theme/studio_theme.h"
...
bool StudioTheme::IsUsingHighContrastTheme() const {
...
gfx::Size StudioTheme::GetThemeBorderSize(ThemeName theme) const {
...
void StudioTheme::DisableTheming() const {
  if (set_theme_properties_)
    set_theme_properties_(0);
}

void StudioTheme::CloseHandles() const {
  if (!close_theme_)
    return;
...
bool StudioTheme::IsClassicTheme(ThemeName name) const {
...
void StudioTheme::OnSysColorChange() {
  UpdateSystemColors();
  is_using_high_contrast_valid_ = false;
  NotifyObservers();
}

void StudioTheme::UpdateSystemColors() {
...
HRESULT StudioTheme::PaintMenuSeparator(
    HDC hdc,
...
HRESULT StudioTheme::PaintMenuArrow(
    HDC hdc,
    State state,
    const gfx::Rect& rect,
...
HRESULT StudioTheme::PaintMenuCheck(
    HDC hdc,
    State state,
    const gfx::Rect& rect,
...
HRESULT StudioTheme::PaintMenuItemBackground(
    HDC hdc,
    State state,
    const gfx::Rect& rect,
...
HRESULT StudioTheme::PaintScrollbarArrow(
    HDC hdc,
    Part part,
    State state,
    const gfx::Rect& rect,
...
HRESULT StudioTheme::PaintScrollbarThumb(
    HDC hdc,
    Part part,
    State state,
    const gfx::Rect& rect,
...
HRESULT StudioTheme::PaintScrollbarTrack(
    SkCanvas* canvas,
    HDC hdc,
    Part part,
    State state,
...
HRESULT StudioTheme::PaintSpinButton(
    HDC hdc,
    Part part,
    State state,
    const gfx::Rect& rect,
...
HRESULT StudioTheme::PaintTrackbar(
    SkCanvas* canvas,
    HDC hdc,
    Part part,
    State state,
...
HRESULT StudioTheme::PaintProgressBar(
    HDC hdc,
    const gfx::Rect& rect,
...
HRESULT StudioTheme::PaintTextField(
    HDC hdc,
    Part part,
    State state,
    const gfx::Rect& rect,
...
// static
StudioTheme::ThemeName StudioTheme::GetThemeName(Part part) {
  switch (part) {
    case kCheckbox:
    case kPushButton:
    case kRadio:
...
HANDLE StudioTheme::GetThemeHandle(ThemeName theme_name) const {
