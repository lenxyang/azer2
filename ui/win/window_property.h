...
#define DECLARE_EXPORTED_WINDOW_PROPERTY_TYPE(EXPORT, T)  \
    template EXPORT void azer::win::Window::SetProperty(         \
        const azer::win::WindowProperty<T >*, T);                \
    template EXPORT T azer::win::Window::GetProperty(            \
        const azer::win::WindowProperty<T >*) const;             \
    template EXPORT void azer::win::Window::ClearProperty(       \
        const azer::win::WindowProperty<T >*);
...
#define DEFINE_WINDOW_PROPERTY_KEY(TYPE, NAME, DEFAULT) \
  ...
  namespace {                                                                 \
    const azer::win::WindowProperty<TYPE> NAME ## _Value = {DEFAULT, #NAME, NULL}; \
  }                                                                           \
  const azer::win::WindowProperty<TYPE>* const NAME = & NAME ## _Value;
...
#define DEFINE_OWNED_WINDOW_PROPERTY_KEY(TYPE, NAME, DEFAULT)   \
  namespace {                                                   \
    void Deallocator ## NAME (int64 p) {                        \
      ...
      delete azer::win::WindowPropertyCaster<TYPE*>::FromInt64(p); \
    }                                                           \
    const azer::win::WindowProperty<TYPE*> NAME ## _Value =          \
