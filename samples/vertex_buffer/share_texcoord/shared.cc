...
VertexBufferPtr InitVertexBuffer(float posx, float posz, float step) {
  const VertexDesc::Desc kVertexDesc[] = {
    {"POSITION", 0, azer::kVec4, 0, 0, false},
  };
  VertexDescPtr desc(new azer::VertexDesc(kVertexDesc, 1));
  int kLineCount = 11;
...
VertexBufferPtr InitVertexBufferTexcoord(int32 row, int column) {
  const VertexDesc::Desc kVertexDesc[] = {
    {"TEXCOORD", 0, azer::kVec2, 0, 0, false},
  };
  VertexDescPtr desc(new azer::VertexDesc(kVertexDesc, 1));
  SlotVertexDataPtr vdata(new SlotVertexData(desc, row * column));
  VertexPack vpack(vdata.get());
  vpack.first();
  float xstep = 1.0 / (float)(column - 1.0f);
  float zstep = 1.0 / (float)(row - 1.0f);
  for (int32 i = 0; i < row; ++i) {
    for (int32 j = 0; j < column; ++j) {
      float x = j * xstep;
      float z = i * zstep;
      vpack.WriteVector2(Vector2(x, z), VertexPos(0, 0)); 
      vpack.next(1);
    }
  }
...
bool MainDelegate::Initialize() { 
  ...
  VertexBufferPtr vb1 = InitVertexBuffer(-10.0f, -10.0f, 1.0f);
