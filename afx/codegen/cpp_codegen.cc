...
std::string CppCodeGen::GenTechnique(const Technique& tech) {
  ...
  ss << "void " << GetClassName(tech) 
     ...
     << "kVertexDescNum);\n"
     << "  const std::string& vs_shader_source = sources[azer::kVertexStage].program;\n"
  ...
  for (int i = (int)(kVertexStage + 1); i < kRenderPipelineStageNum; ++i) {
    ...
    ss << "  {\n"
       << "    const std::string& source = sources[" << StageName(stage) << "].program;\n"
       << "    const std::string& path = sources[" << StageName(stage) << "].path;\n"
       << "    DCHECK(!source.empty());\n"
