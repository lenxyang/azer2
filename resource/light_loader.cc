...
namespace {
bool LoadAttenuation(Vector3* atten, float* range, const ConfigNode* config) {
  ...
  CHECK(atten_node->GetChildTextAsVec3("coefficient", atten));
  CHECK(atten_node->GetChildTextAsFloat("range", range))
      << "attenuation node has distance";
  return true;
}
}  // namespace

const char LightLoader::kSpecialLoaderName[] = "azer::kSpecialLoaderName";
...
bool LightLoader::CouldLoad(ConfigNode* node) const {
...
VariantResource LightLoader::Load(const ConfigNode* lnode,
  ...
  if (light_type == "point_light") {
    ...
    CHECK(LoadAttenuation(&light.coeff, &light.range, lnode));
